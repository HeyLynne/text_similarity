...
from data_processor import build_glove_dic
...
class TextCNN(object):
    ...
    def init_weight(self):
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            """
            Embedding layer, transform tensor from a bivector to a trivector
            """
            ...
            self.s1 = tf.nn.embedding_lookup(self.W, self.input_s1)# shape: batch size, sequence length, embedding _size
            self.s2 = tf.nn.embedding_lookup(self.W, self.input_s2)
            self.x = tf.concat([self.s1, self.s2], axis = 1)
            self.x = tf.expand_dims(self.x, -1) # shape: batch_size, sequence_length * 2, embedding_size, 1
    ...
    def inference(self):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("conv-maxpool%s" % filter_size):
                ...
                conv = tf.nn.conv2d(self.x, W, strides = [1, 1, 1, 1], padding = "VALID", name = "conv")# shape: batch_size, seuqence_length * 2 - filter_size + 1, 1, num_filters
                h = tf.nn.relu(tf.nn.bias_add(conv, b), name = "relu")
                pooled = tf.nn.max_pool(h, ksize = [1, self.sequence_length * 2 - filter_size + 1, 1, 1], strides = [1, 1, 1, 1], padding = 'VALID', name = 'pool')
    ...
    def add_output(self):
        with tf.name_scope("output"):
            W = tf.get_variable("W", shape = [self.num_filters_total, self.num_classes], initializer = tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_acc(self):
        ...
        with tf.name_scope("pearson"):
            mid1 = tf.reduce_mean(self.scores * self.input_y) - tf.reduce_mean(self.scores) * tf.reduce_mean(self.input_y)
